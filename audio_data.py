...
class WavenetDataset(torch.utils.data.Dataset):
    ...
    def normalize_audio(x):
        return np.clip(x / np.max(np.abs(x)), -1, 1) # need to clip the audio as well afterwards in case of rounding errors
    ...
    def create_dataset(self, location, out_file):
        ...
        for i, file in enumerate(files):
            ...
            file_data, sr = sf.read(file)
            
            # Convert to mono if needed
            if self.mono and file_data.ndim > 1:
                file_data = np.mean(file_data, axis=1)
    
            # Resample if needed
            file_data = resample_audio(file_data, sr, self.sampling_rate)
    
            if self.normalize:
                file_data = normalize_audio(file_data)
